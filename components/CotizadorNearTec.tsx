...
const simplifiedServices: Array<{ value: ServiceFocus; title: string; copy: string; icon: string }> = [
  { value: 'diseno', title: 'Sitio web', copy: 'Para presentar y vender mejor.', icon: '🌐' },
  ...
  { value: 'personalizado', title: 'Algo mixto', copy: 'Para una necesidad especial.', icon: '✨' },
]
...
export default function CotizadorNearTec() {
  ...
  const pricingMessage =
    serviceFocus === 'compunegocio' || serviceFocus === 'cn7'
      ? 'Aquí sí te mostramos una base real de precio.'
      : 'Este servicio se cotiza según alcance. Aquí te ayudamos a dejar clara la necesidad antes de hablar con ventas.'

  const recurringLabel = quote.monthlyRecurringLabel ?? quote.annualRecurringLabel
  const economicLabel = recurringLabel ?? (quote.oneTimeMxn > 0 ? formatMoney(quote.oneTimeMxn, 'MXN') : 'Se valida por alcance')
  const nextStep =
    qualification.tone === 'hot'
      ? 'Lo mejor es pasar directo a WhatsApp con esta información.'
      : qualification.tone === 'warm'
        ? 'Lo mejor es revisar contigo el alcance y resolver dudas.'
        : 'Lo mejor es usar esta base para orientarte antes de cotizar.'
  ...
  const whatsappMessage = [
    ...
    `CN7: ${cloudPlan === 'cn7_backup' ? 'Con respaldo' : cloudPlan === 'cn7_hosted' ? 'Hospedado' : 'No aplica'}`,
    ...
    <section className="grid gap-6 lg:grid-cols-[1.04fr_0.96fr]">
      <div className="overflow-hidden rounded-[32px] border border-[#dce8bf] bg-white shadow-[0_28px_70px_rgba(15,17,21,0.08)]">
        <div className="border-b border-[#edf2df] bg-[linear-gradient(180deg,#ffffff_0%,#f7faef_100%)] px-5 py-5 sm:px-6">
          ...
          <h3 className="mt-3 text-2xl font-black text-[#0f1115] sm:text-[2rem]">Elige lo que necesitas y te damos una base clara.</h3>
          <p className="mt-2 max-w-2xl text-sm leading-7 text-[#67717a]">
            Este cotizador está hecho para entender rápido el servicio correcto y facilitar la conversación con ventas.
          </p>
        </div>
        ...
        <div className="overflow-hidden rounded-[32px] border border-[#dce8bf] bg-[linear-gradient(180deg,#ffffff_0%,#f7faef_100%)] shadow-[0_28px_70px_rgba(15,17,21,0.1)]">
          <div className="border-b border-[#edf2df] px-5 py-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <span className="rounded-full border border-[#dce8bf] bg-[#eef7d7] px-4 py-2 text-sm font-black text-[#0f1115]">{qualification.label}</span>
              <span className="rounded-full bg-[#0f1115] px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-white">{service?.label}</span>
            </div>
          ...
          <div className="space-y-5 px-5 py-5">
            <div className="rounded-[24px] border border-[#e6e8ea] bg-white p-4 shadow-sm">
              ...
              <p className="mt-2 text-sm leading-7 text-[#67717a]">
                {quote.oneTimeMxn > 0 && recurringLabel ? 'Puede incluir cargo único y cargo recurrente.' : 'El asesor valida el alcance final contigo.'}
              </p>
            </div>
            ...
            <div className="rounded-[28px] border border-[#0f1115] bg-[#0f1115] p-4 text-white shadow-[0_18px_38px_rgba(15,17,21,0.2)]">
              ...
              <p className="mt-2 text-sm leading-7 text-white/78">Así la conversación llega más clara y más rápida a WhatsApp o correo.</p>
              <div className="mt-4 grid gap-3">
                <button type="button" onClick={() => openWhatsApp(whatsappMessage)} className="rounded-full bg-[#9ac43b] px-5 py-3 text-sm font-black uppercase tracking-[0.12em] text-[#0f1115] transition hover:translate-y-[-1px]">Enviar a WhatsApp</button>
                <a href={`mailto:${CONTACT.email}?subject=${encodeURIComponent('Cotización NearTec')}&body=${encodeURIComponent(whatsappMessage)}`} className="rounded-full border border-white/14 px-5 py-3 text-center text-sm font-black uppercase tracking-[0.12em] text-white transition hover:bg-white/5">Enviar por correo</a>
              </div>
            </div>
          </div>
        </div>
