...
BRANCH="${BRANCH:-main}"
COMMIT_MESSAGE="${COMMIT_MESSAGE:-Replace NearTec with V4.1 sales engine}"
...
BACKUP_BRANCH="backup/pre-v41-$(date +%Y%m%d-%H%M%S)"; git checkout -b "$BACKUP_BRANCH"; git push -u origin "$BACKUP_BRANCH"; git checkout "$BRANCH"
...
rsync -a --delete --exclude='.git/' --exclude='.vercel/' --exclude='node_modules/' --exclude='.next/' "$SRC"/ "$REPO_DIR"/
chmod +x scripts/*.sh || true
npm install --engine-strict=false
npm run predeploy:check
npm run smoke
if [ -n "${PREFIX:-}" ] && echo "$PREFIX" | grep -q "com.termux"; then
  echo "Termux/Android detectado: se omite npm run build local porque Next.js SWC compila en Vercel/Linux."
else
  npm run build
fi
git add -A
