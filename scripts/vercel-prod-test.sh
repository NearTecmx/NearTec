...
check_200() {
  local path="$1"
  local code
  code="$(curl -L -sS -o /dev/null -w "%{http_code}" "$PROJECT_URL$path")"
  if [ "$code" != "200" ]; then
    echo "ERROR $path esperado 200, obtuvo $code"
    exit 1
  fi
  echo "OK 200 $path"
}
...
API_GET_CODE="$(curl -L -sS -o /dev/null -w "%{http_code}" "$PROJECT_URL/api/lead")"
if [ "$API_GET_CODE" != "405" ]; then
  echo "ERROR API GET esperado 405, obtuvo $API_GET_CODE"
  exit 1
fi
...
echo "$API_RESPONSE"
echo "$API_RESPONSE" | grep -q '"ok":true' || { echo "ERROR API POST no respondió ok:true"; exit 1; }
